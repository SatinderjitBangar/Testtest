...
public class RE_PersonalInfoPage {

	public WebDriver ldriver;

	// Constructor
	public RE_PersonalInfoPage(WebDriver rdriver) {
		ldriver = rdriver;
		PageFactory.initElements(rdriver, this);
	}

	// Capture Personal Info webelement
	@FindBy(xpath = "//h2[contains(text(),'Personal Info')]")
	WebElement txtPersonalInfo;
	
	//Capture the link Personal info in tab
			@FindBy(xpath="//a[contains(@class,'v-tab')][1]")
			@CacheLookup
			public WebElement lnkPersonalInfo;
			
			//Capture the First name text box in personal info form
			@FindBy(xpath="//div[@class='wrapper df']/div[1]//span[1]//input")
			@CacheLookup
			public WebElement txtFirstName;
					
			//Capture the Last name text box in personal info form
			@FindBy(xpath="//div[@class='wrapper df']/div[1]//span[2]//input")
			@CacheLookup
			public WebElement txtLastName;
					
			//Capture the Preferred name text box in personal info form
			@FindBy(xpath="//div[@class='wrapper df']/div[1]//span[3]//input")
			@CacheLookup
			public WebElement txtPrefName;
			
			//Capture the Date of Birth Date picker in personal info form
			...
			//Capture the Phone text box in personal info form
			@FindBy(xpath="//div[@class='wrapper df']/div[1]//span[5]//input")
			@CacheLookup
			public WebElement txtPhone;
			
			//Capture the Driver License/others text box in personal info form
			@FindBy(xpath="//div[@class='wrapper df']/div[2]//span[1]//input")
			@CacheLookup
			public WebElement txtDl;
			
			//Capture the State/Province text box in personal info form
			...
			//Capture the Social Security text box in personal info form
			@FindBy(xpath="//div[@class='wrapper df']/div[2]//span[4]//input")
			@CacheLookup
			public WebElement txtSSN;
			
			//Capture the Next button in personal info form
			@FindBy(xpath="//button[@class='v-btn v-btn--contained theme--light v-size--default teal']")
			@CacheLookup
			...
			@FindBy(xpath="//p[contains(text(),\"The Driver's License / Others field\")]")
			@CacheLookup
			...
			//Capture Co-Applicant Radio button
			@FindBy(xpath="//input[@value=\"radio-2\"]/following-sibling::div")
			@CacheLookup
			public WebElement btnCoApp;
			
			//Capture Single-Applicant Radio button
			@FindBy(xpath="//input[@value=\"radio-1\"]/following-sibling::div")
			@CacheLookup
			public WebElement btnSingleApp;


	
	// =======================ACTION METHOD=============================//

	//Check whether personal info is displayed or not
	public boolean verifyPersonalInfoPresent() {
		if (txtPersonalInfo.isDisplayed()) {
			return true;
	...
	//Set the First Name text box
			...
			try {
		        wait.until(expectation);
		        wait.until(ExpectedConditions.visibilityOf(txtSSN));
				wait.until(ExpectedConditions.elementToBeClickable(txtSSN));
		        txtSSN.click();
			...
			public void clkbtnCoApp() {
				WebDriverWait wait = new WebDriverWait(ldriver, 25);
				wait.until(ExpectedConditions.visibilityOf(btnCoApp));
				wait.until(ExpectedConditions.elementToBeClickable(btnCoApp));
				btnCoApp.click();
				}
			
			//Click on Single Applicant radio button
			public void clkbtnSingleApp() {
				WebDriverWait wait = new WebDriverWait(ldriver, 25);
				wait.until(ExpectedConditions.visibilityOf(btnSingleApp));
				wait.until(ExpectedConditions.elementToBeClickable(btnSingleApp));
				btnSingleApp.click();
				}
